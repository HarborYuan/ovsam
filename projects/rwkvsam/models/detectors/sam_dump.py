...
class BackboneDump(BaseModel):
    ...
    def predict(self, batch_inputs: Tensor,
        ...
        dir_path = os.path.dirname(batch_data_samples[0].metainfo['img_path'])
        dir_path = os.path.dirname(dir_path)
        dir_path = dir_path.replace('sam', 'sam_feat')

        img_path = os.path.basename(batch_data_samples[0].metainfo['img_path'])
        ...
        if not mmengine.exists(img_path):
            ...
            print_log(f'{img_path} already exists, but still regenerate.')
            feat = feat.to(device='cpu')[0]
            torch.save(feat.to(device='cpu'), img_path)
        return {}
